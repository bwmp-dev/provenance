...
test("generation uses only local plugins with unchanged options and exact pins", () => {
  ...
  assert.equal(
    ...
    "node scripts/sync-config-schemas.mjs && node scripts/sync-execution-evidence-openapi.mjs && node scripts/generate-protobuf.mjs && pnpm --filter @bwmp-dev/api-client generate",
  );
});
