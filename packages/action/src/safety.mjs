...
export function https(value, originOnly = false) {
  ...
  if (
    ...
    u.password ||
    u.hash ||
    (originOnly && (u.pathname !== "/" || u.search))
  )
    fail("invalid_configuration");
  return u;
}
...
// JSON.parse alone accepts duplicate members and replacement-decoded UTF-8.
// Validate lexical structure first so authority/identity cannot be ambiguous.
export function strictJSON(bytes) {
  try {
    ...
    const string = () => {
      ...
      if (!value.isWellFormed()) fail("invalid_response");
      return value;
    };
    const value = (depth) => {
