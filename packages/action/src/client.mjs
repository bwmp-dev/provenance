...
export const reportingState = (outcome) =>
  descriptions[outcome] || descriptions.incomplete;
...
export function validateInputs(c) {
  ...
  if (
    ...
    !sha(c.commit) ||
    typeof c.ref !== "string" ||
    !c.ref.startsWith("refs/") ||
    c.ref.length > 512 ||
    /[\r\n\x00]/.test(c.ref) ||
    typeof c.audience !== "string" ||
    !c.audience ||
    c.audience.length > 2048 ||
    typeof c.repository !== "string" ||
    !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(c.repository) ||
    !numeric(c.repositoryId) ||
    !numeric(c.ownerId) ||
    typeof c.version !== "string" ||
    !c.version ||
    c.version.length > 128 ||
    typeof c.changelog !== "string" ||
    c.changelog.length > 65536 ||
    !["none", "status"].includes(c.report) ||
    typeof c.wait !== "boolean"
  )
  ...
  if (
    c.requestMs > c.totalMs ||
    (c.report === "status" &&
      (typeof c.repositoryToken !== "string" || !c.repositoryToken))
  )
    fail("invalid_configuration");
  return c;
}
...
export async function runAction(input, runtime) {
  ...
  const report = async (state) => {
    ...
    if (
      !object(body) ||
      body.state !== value ||
      body.context !== "provenance/action"
    )
  ...
  try {
    ...
    if (
      !runtime.testEndpoints &&
      !oidcURL.hostname.endsWith(".actions.githubusercontent.com")
    )
    ...
    if (
      ...
      oidc.value.length < 16 ||
      oidc.value.length > 16384 ||
      !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(oidc.value)
    )
      fail("invalid_response");
    mask(oidc.value);
    ...
    if (
      !uuid(grant.grantId) ||
      !uuid(grant.scope.organizationId) ||
      grant.principalType !== "github-actions" ||
      grant.tokenType !== "Bearer" ||
      !/^pva_[A-Za-z0-9_-]{42}[AEIMQUYcgkosw048]$/.test(grant.accessToken) ||
      grant.scope.projectId !== config.project ||
      grant.scope.repositoryId !== config.repositoryId ||
      grant.scope.repositoryOwnerId !== config.ownerId ||
      grant.scope.sourceCommit !== config.commit ||
      grant.scope.sourceRef !== config.ref ||
      !numeric(grant.scope.appId) ||
      !numeric(grant.scope.installationId) ||
      typeof grant.scope.workflowRef !== "string" ||
      !grant.scope.workflowRef ||
    ...
    for (;;) {
      ...
      if (
        ...
        artifact.projectId !== config.project ||
        artifact.sha256 !== jar.digest ||
        artifact.sizeBytes !== jar.bytes.length
      )
        fail("identity_mismatch");
    ...
    const verifyCandidate = (v) => {
      ...
      if (
        !uuid(v.id) ||
        (candidateId && v.id !== candidateId) ||
        v.projectId !== config.project ||
        v.artifactId !== artifactId ||
        v.configurationHash !== configurationHash ||
        ...
        for (let pageNo = 0; ; pageNo++) {
          ...
          if (page.page.nextCursor != null) {
            if (
              ...
              page.page.nextCursor.length > 2048 ||
